...
# 13,627 raw docs (vs 776 at window_days=3) because pick_tracking writes a
# fresh row per symbol every ~15min cron cycle, so a wider window means
# reading many more cycles' worth of rows before collapsing them down to
# one row per symbol -- 12-14s per request, identical for every caller
# hitting "More" (AllPicksScreen's WINDOW_DAYS=30), so one shared cache
# entry serves all of them instead of every request re-paying that scan.
# Does NOT cache current_price/quote data -- that's fetched fresh from
# get_stocks() after this cache, on every request, so live prices are
...
def get_alphaclara_tracking(
    ...
    try:
        ...
        if cache_fresh:
            ...
            latest_resolved_by_symbol = cached_window["latest_resolved_by_symbol"]
