...
def compute_indicator_states(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ------------------------------------------------------------
    # Ensure all 68 first-order indicator states exist
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # SECOND-ORDER INTERPRETED STATES (for narration)
    # (derived, reasoning-level states)
    # ------------------------------------------------------------

    states["momentum_composite"] = _combine_momentum(
        states.get("rsi14", "UNKNOWN"),
        states.get("macd_hist", "UNKNOWN"),
    )

    states["probability_composite"] = _combine_probability(
        states.get("hybrid_prob_up", "UNKNOWN"),
        states.get("hybrid_prob_down", "UNKNOWN"),
    )

    states["volatility_composite"] = _combine_volatility(
        states.get("volatility_20d", "UNKNOWN"),
        states.get("intraday_range_pct", "UNKNOWN"),
    )
...
# ============================================================
# SECOND-ORDER STATE HELPERS (MODULE-LEVEL, PURE)
# ============================================================

def _combine_momentum(rsi_state: str, macd_state: str) -> str:
    if "BULL" in macd_state and rsi_state in ("BULLISH", "OVERBOUGHT"):
        return "MOMENTUM_BULLISH"
    if "BEAR" in macd_state and rsi_state in ("BEARISH", "OVERSOLD"):
        return "MOMENTUM_BEARISH"
    if "OVERBOUGHT" in rsi_state and "BULL" in macd_state:
        return "MOMENTUM_BULL_STRETCHED"
    if "OVERSOLD" in rsi_state and "BEAR" in macd_state:
        return "MOMENTUM_BEAR_STRETCHED"
    return "MOMENTUM_MIXED"


def _combine_probability(up_state: str, down_state: str) -> str:
    if up_state in ("VERY_HIGH", "HIGH") and down_state in ("LOW", "VERY_LOW"):
        return "PROB_STRONGLY_UP"
    if down_state in ("VERY_HIGH", "HIGH") and up_state in ("LOW", "VERY_LOW"):
        return "PROB_STRONGLY_DOWN"
    if up_state in ("HIGH", "LEAN_HIGH"):
        return "PROB_WEAKLY_UP"
    if down_state in ("HIGH", "LEAN_HIGH"):
        return "PROB_WEAKLY_DOWN"
    return "PROB_BALANCED"


def _combine_volatility(vol_state: str, intraday_state: str) -> str:
    if vol_state == "HIGH_VOL" or intraday_state in ("VERY_WIDE", "EXTREME"):
        return "VOLATILITY_EXPANDING"
    if vol_state == "LOW_VOL" and intraday_state == "TIGHT":
